...
class MMFi_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        signal = cv2.resize(signal, (136, 32), interpolation=cv2.INTER_NEAREST)
        signal = signal.transpose(2, 1, 0)
        output = signal
        input = self.add_noise(signal)


        sample["input_noise"] = input
        sample["output"] = output

        sample['input_noise_norm'] = self.min_max_scaling(sample['input_noise'])
        sample['output_norm'] = self.min_max_scaling(sample['output'])

        return torch.tensor(sample["input_noise_norm"]).float(), torch.tensor(sample["output_norm"]).float()
...
if __name__ == '__main__':
    with open("/src/denoise/configs/config.yaml", 'r') as f:
        cfg = yaml.load(f, Loader=yaml.FullLoader)
    train_dataset, val_dataset = make_dataset("/home/nxhoang/HPE/Data", cfg)

    print(train_dataset[0])
    print(train_dataset[0]['output'].shape)
    print(train_dataset[0]['input_noise'].shape)
    pass